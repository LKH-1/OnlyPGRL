    ...
    def __init__(self, name: str, state_size, action_size, temp=0.1):
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('policy_net'):
                layer_1 = tf.layers.dense(inputs=self.obs, units=64, activation=tf.tanh, kernel_initializer=tf.contrib.layers.xavier_initializer())
                layer_2 = tf.layers.dense(inputs=layer_1, units=64, activation=tf.tanh, kernel_initializer=tf.contrib.layers.xavier_initializer())
                layer_3 = tf.layers.dense(inputs=layer_2, units=64, activation=tf.tanh, kernel_initializer=tf.contrib.layers.xavier_initializer())
            ...
            with tf.variable_scope('value_net'):
                layer_1 = tf.layers.dense(inputs=self.obs, units=64, activation=tf.tanh, kernel_initializer=tf.contrib.layers.xavier_initializer())
                layer_2 = tf.layers.dense(inputs=layer_1, units=64, activation=tf.tanh, kernel_initializer=tf.contrib.layers.xavier_initializer())
                layer_3 = tf.layers.dense(inputs=layer_2, units=30, activation=tf.tanh, kernel_initializer=tf.contrib.layers.xavier_initializer())
